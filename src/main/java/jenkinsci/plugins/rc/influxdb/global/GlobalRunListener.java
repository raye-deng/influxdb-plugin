package jenkinsci.plugins.rc.influxdb.global;
...
import jenkinsci.plugins.rc.influxdb.InfluxDbPublicationService;
import jenkinsci.plugins.rc.influxdb.InfluxDbStoreStep;
...
public class GlobalRunListener extends RunListener<Run<?, ?>> {
    ...
    public void onCompleted(Run<?, ?> build, @Nonnull TaskListener listener) {
        ...
        List<Target> targets = jenkins.getDescriptorByType(InfluxDbStoreStep.DescriptorImpl.class).getTargets();
    ...
    private boolean isPublicationInBuild(Target target, Run<?, ?> build) {
        ...
        if (parent instanceof AbstractProject) {
            InfluxDbStoreStep publisher = (InfluxDbStoreStep) ((AbstractProject) parent).getPublishersList().get(InfluxDbStoreStep.class);
            if (publisher != null) {
