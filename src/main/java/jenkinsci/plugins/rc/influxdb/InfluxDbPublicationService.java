...
import jenkinsci.plugins.rc.influxdb.generators.*;
import jenkinsci.plugins.rc.influxdb.generators.*;
import jenkinsci.plugins.rc.influxdb.generators.serenity.SerenityJsonSummaryFile;
import jenkinsci.plugins.rc.influxdb.generators.serenity.SerenityPointGenerator;
import jenkinsci.plugins.rc.influxdb.models.Target;
import jenkinsci.plugins.rc.influxdb.renderer.ProjectNameRenderer;
...
public class InfluxDbPublicationService {
    ...
    public void perform(Run<?, ?> build, TaskListener listener, EnvVars env) {
        ...
        for (int i = 0; i < this.customDataMapList.size(); i++) {
            ...
            this.handleCustomDataWrite(pointsToWrite, build, listener, measurementRenderer, timestamp + (i * 1L), jenkinsEnvParameterTag, customPrefix, dataMap, dataMapTags);
        }
        try {
        ...
        for (Target target : selectedTargets) {
            ...
            try (InfluxDBClient influxDB = getInfluxDBClient(build, target, url)) {
                ...
                this.printStepLog(listener, "[InfluxDB Plugin][Error] influxdb write failed, error message:" + e.getMessage());
    ...
    private void addPoints(List<Point> pointsToWrite, PointGenerator generator, TaskListener listener) {
        try {
            ...
            if (this.quiet) {
                listener.getLogger().println("[InfluxDB Plugin] Failed to collect data. Ignoring Exception:" + e.getMessage());
            } else {
                throw e;
            }
        }
